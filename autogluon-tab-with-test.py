...
os.system('pip install autogluon==0.0.15 seaborn matplotlib')
...
def train(args):
    ...
    hosts = args.hosts
    time_limits = int(args.training_minutes) * 60
    ...
    predictor = task.fit(train_data=train_data, label=target, 
        output_directory=model_dir, eval_metric=eval_metric, 
        excluded_model_types=['KNN','RF','NN'],
        time_limits=time_limits, presets=[presets, 'optimize_for_deployment'])
...
def evaluate(predictor, args):
    ...
    files_to_upload = [pred_file, lead_file, perf_file, summ_file, eval_file, featimp_imgfile, featimp_csvfile]
